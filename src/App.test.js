...
test('renders data after API is called', () => {
    fetchShow.mockResolvedValueOnce(showData.data);

    render(<App />);

    waitForElementToBeRemoved(screen.queryByText(/Fetching data.../i))
        .then(() => {
            expect(screen.queryByTestId(/showName/i)).toBeInTheDocument();
        })
...
test('renders all available seasons from data API', async () => {
    ...
    await waitFor(() => expect(screen.queryAllByText(/season /i)).toHaveLength(4));
    expect(fetchShow).toHaveBeenCalledTimes(3);
});